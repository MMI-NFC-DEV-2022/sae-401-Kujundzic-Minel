    ...
    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
      <div v-for="film in films" :key="film.id" class="rounded-lg overflow-hidden shadow-lg">
        <router-link :to="`/films/${film.id}`" class="block">
          <img :src="film.film_cover" alt="Image du film" class="object-cover h-60 w-full" />
          <h2 class="text-xl font-semibold p-4">{{ film.film_name }}</h2>
        </router-link>
      </div>
    </div>
    <div class="mt-6 flex justify-center">
      <button class="bg-[#E3E8F2] rounded-lg px-4 py-2 font-semibold">Voir tous les films</button>
    </div>
